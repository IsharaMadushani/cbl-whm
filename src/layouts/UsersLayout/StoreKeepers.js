...
export default function StoreKeepers() {
    ...
    React.useEffect(() => {
        firebase.database().ref('users/storekeepers').on('value', (snapshot) => {
            var users = [];
            if (snapshot.exists()) {
                        ...
                        setTimeout(() => {
                            ...
                            firebase.auth().createUserWithEmailAndPassword(newData.email, 'ucsc@123')
                                .then((userData) => {
                                    ...
                                    firebase.database().ref('users/storekeepers').child(userData.user.uid).set(newData)
                                    console.log(newData)
                                })
                            // setState((prevState) => {
                ...
                onRowUpdate: (newData, oldData) =>
                    new Promise((resolve) => {
                        setTimeout(() => {
                            resolve();
                            if (oldData) {
                                //firebase.database().ref('users/storekeepers').push(newData)
                                firebase.database().ref('users/storekeepers').child(`${oldData.key}`).remove()
                                firebase.database().ref('users/storekeepers').child(oldData.key).set(newData)
                                setState((prevState) => {
                                    const data = [...prevState.data];
